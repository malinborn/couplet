...
- **Теги не пишутся внутрь файлов**, никакого frontmatter. Файл пользователя остаётся байт-в-байт тем же, иначе git покажет diff от couplet. Метаданные лежат в одном индексе стэша.
...
Заметки — обычные `.md`-файлы, по одному на заметку, запись через `atomic_write::save`. Индекс стэша (теги, отметки «отложено», ссылки на файлы) — один файл рядом с заметками.
...
Сессия хранит **ссылки** на заметки, но не владеет ими: сборщик мусора сессии к папке стэша не прикасается ни при каких условиях.
...
- **Быстрые теги (#8):** «тег N» по клавише с цифрой. Клавиша условная; `⌘⇧T` занят (возврат закрытой вкладки), нужна другая, например `⌃⇧1…9` через страницу.
- Фильтр по тегам — в той же строке поиска, что и по тексту: `#tag` в запросе.

### 4.5 Где стэш живёт в UI

Ответ из заметок: **общий для приложения, отдельно от вкладок**. Вкладки — это то, что сейчас открыто, стэш — то, что хранится. Смешать их — снова размыть разницу между «на виду» и «сохранено», из-за которой всё и случилось.

Две формы из блокнота (#10), обе стоит попробовать на прототипе:

| | Второй режим дровера | Стэш как «окно» в карусели |
|---|---|---|
| Как открыть | `⌘J` → переключить на «Стэш», или своя клавиша сразу в стэш | виден, когда тащишь карточку или жмёшь `⌘G` «В окно…» |
| «Отложить» мышью | кнопка на карточке | перетащить вкладку на стэш — тот же жест, что перенос в другое окно |
| Новая механика | фильтр, теги, список | почти никакой: карусель уже есть |
| Чего не хватает | — | просмотра стэша без переноса |

Похоже, они не конкуренты: карусель — способ **положить** в стэш, режим дровера — способ **найти и достать**. Тогда ответ на вопрос из блокнота «нужно ли открывать стэш вместе с дровером» — нет, достаточно переключателя режимов.

**Отметка «stashed» (#4):** в карточке вкладки и в заголовке окна у заметки — значок стэша вместо «Untitled».

**Режимы и цвет (из блокнота):** «stash modes? with different colouring?» — открыто, см. вопросы.
...
Семантический поиск по стэшу как расширение или плагин — отдельная фаза. Стоит проектировать индекс стэша так, чтобы к нему можно было приложить векторы, но в первую версию не брать.

## 5. Фаза 0 — страховка, которую стоит сделать сразу

Независимо от стэша, одним маленьким PR:
...
1. **Папка:** `~/Documents/couplet/` (видна, Spotlight, переживает zap) или скрытая Application Support?
2. **Форма UI:** начинаем с режима дровера, а карусель добавляем как способ положить — или прототипируем обе сразу и выбираем на ощупь?
3. **«Режимы стэша с разным цветом»** — что имелось в виду: цвет по тегу, цвет «отложено» против «просто в стэше», или отдельные стэши?
4. **Клавиши:** `⌃T` = отложить подходит? Для быстрых тегов — `⌃⇧1…9` или другое?
5. **Фаза 0** — делаем сейчас, до стэша?
