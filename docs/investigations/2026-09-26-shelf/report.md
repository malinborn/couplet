...
- **Ширина.** В обычном окне оба дровера помещаются рядом: дровер вкладок занимает меньше половины широкого окна. В узком окне (ниже порога, который найдём на макете) дровер стэша выезжает поверх дровера вкладок вторым слоем.
...
2. **Клавиши:** `⌃T` = отложить подходит? Для быстрых тегов — `⌃⇧1…9` или другое? И нужна ли клавиша, открывающая дровер стэша сразу, без дровера вкладок?
3. **Фаза 0** — делаем сейчас, до стэша?
