...
>
> Версия 4: по макету v2 — дроверы никогда не перекрываются, в узком окне компактны, а слишком узкое окно само раздвигается; открытое вкладкой в окне не показывается в его тайнике (4.5). Быстрые теги отложены (4.4). Фаза 0 объяснена заново (5). Макет: `docs/investigations/2026-09-26-stash-mockup/stash-drawers.html`.
>
> Версия 5: по-русски фича называется **«тайник»**, по-английски остаётся **stash** (Max). Своя корзина couplet с разделом «Удалённые» внизу дровера тайника; секций «Отложено / Остальное» нет — один список, а время «отложено» есть на каждой карточке (4.5).
...
- **Решено (Max):**
  - все документы по умолчанию лежат в тайнике: хрупкого untitled больше нет;
  - отдельный жест «отложить» убирает документ с глаз, но оставляет его в тайнике под рукой;
  - тайник общий для всего приложения, с тегами, и окно проекта само фильтрует тайник по тегу своего репо;
  - агент видит тайник через методы list / get / add;
  - **хранение:** SQLite — источник правды для тайника (записи, теги, ссылки), текст заметок лежит `.md`-файлами, путь к ним записан в БД;
  - **интерфейс:** внизу дровера вкладок — область тайника с кнопкой; кнопка открывает справа второй дровер своего цвета, оба открыты одновременно; пока тащишь вкладки, нижняя область превращается в зону сброса «в тайник»;
  - **дедупликация:** один и тот же документ, отложенный из разных окон, — одна запись.
- **Правило для файлов (предложено, Max принял к записи):** тайник хранит заметки и ссылки на файлы. Тег на файле = отложить его в тайник. Сами файлы couplet не меняет.
- **До тайника стоит сделать фазу 0:** сборщик мусора сессии больше не удаляет черновики, а переносит их в корзину. Это полдня работы, и это уже спасло бы план.
...
**Карусель (#10).** Тайник как ещё одно «окно» в карусели остаётся возможным вторым способом положить в тайник. С зоной сброса внизу дровера он, скорее всего, не нужен — проверим на макете. Вопрос из блокнота «нужно ли открывать тайник вместе с дровером» закрыт: да, два дровера рядом.
...
Закрыто в версии 3: форма UI (два дровера + зона сброса), «режимы с разным цветом» (свой цвет у дровера тайника), хранение (SQLite + файлы), клавиатура при двух дроверах.
