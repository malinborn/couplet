...
| D4 | A background file tab is always clean: leaving a file-backed tab **first flushes autosave (with bounded retries) and is refused only if that save did not land** (a standing `save-error` / `unsaved-blocked`, or still dirty after the retries) — same rule as plan 01's switch. A keystroke inside the 300 ms debounce is never a reason to refuse (architect review; test `SwitchingRightAfterTypingFlushesAndSucceeds_NoRefusal`). Background *untitled* tabs may hold text. | A cached dirty file state would have no autosave, no recovery snapshot and no watcher — the exact silent-loss path roast 1b warns about. |
...
| D7 | **AI command for a path held by a background tab** (interim until plan 04): the window activates that tab through the normal path (which first flushes the active tab's autosave), then handles the command as for the active tab. Refused with an error only if that flush did not land (save error / still dirty after retries), or if the active tab shows a live `ask` from an agent (an agent never yanks the view away from another agent's question). | Simplest behaviour that is correct: the command lands in a live state with undo, highlight and widgets. The "never steal from a live question" rule costs one line and protects the case plan 04 will solve properly. |
...
- **An `ask` does not survive its tab going to the background**; it is cancelled with an error (D6). This is acceptable only as an interim that ships in the same release as plan 04. **Hand-off to plan 04 (must be its own task there):** "an `ask` survives its tab going to the background — the tab shimmers (unviewed), the question is delivered when the tab is activated, and the agent receives no error" (spec §5).
