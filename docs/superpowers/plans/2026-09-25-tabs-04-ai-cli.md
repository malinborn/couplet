...
| D3 | **Focus defaults:** `show` → `focus: true` unless the call says otherwise (spec §5 names `true` the default; it is also what every `show` did before, so old callers keep their behaviour). An agent's open (`mdmini <file>` with `CLAUDECODE` set) → background; a human's `-t N` → focus; `-b`/`-f` override either. **`edit` and `ask` never switch tabs** (spec §5: an edit of a background tab applies there; an ask waits there). A new window created for a background command is built **without activating the app** (`focused(false)`, no `NSApp activate`, spec §4). | The brief summarised §5 as "background by default"; the spec's text makes that true for opens, edits and asks and false for `show`. Flip point if the architect prefers background shows: `AiRequest::focus()` and the MCP schema default — two lines (open question 1). |
...
| `show` focus default vs "agent opens in background" | §5 `focus` default `true`; §4 agent `coup file` → background | both, per verb (D3); open question 1 |
...
````markdown
## Q6. `show` по умолчанию открывает вкладку в фокусе — так и оставить?
- **Контекст:** план 04, решение D3. Спека §5: у MCP `show` параметр `focus` по умолчанию `true`; §4: открытие файла агентом (`coup file`) — в фоне. План сделал ровно так: `show` без `focus` переключает на вкладку и выводит окно вперёд (как и до вкладок), `mdmini file` от агента, `edit` и `ask` — в фоне. Вкладку, в которой ты печатаешь, не отнимает никто и никогда.
- **Альтернативы:** 1) оставить; 2) `show` по умолчанию в фоне — две строки (`AiRequest::focus()` в `ai_socket.rs` и `default` в схеме MCP), но тогда «покажи мне» от агента перестанет выводить документ вперёд.
...
replaces the interim tabs section; CLAUDE.md gotchas; Q4 stays open, Q6–Q9
...
- Create: `docs/superpowers/plans/night-shots/tabs04-dev-*.png`
...
```bash
git add docs/superpowers/plans/night-shots/tabs04-dev-01-routed.png docs/superpowers/plans/night-shots/tabs04-dev-02-show-after-typing.png docs/superpowers/plans/night-shots/tabs04-dev-03-bg-edit.png docs/superpowers/plans/night-shots/tabs04-dev-04-ask-waiting.png docs/superpowers/plans/night-shots/tabs04-dev-05-quick-look.png docs/superpowers/plans/night-shots/tabs04-dev-06-human-open.png
git commit -m "$(cat <<'EOF'
docs(tabs): plan 04 dev QA shots

Co-Authored-By: Claude Opus 5.5 (1M context) <noreply@anthropic.com>
EOF
)" -- docs/superpowers/plans/night-shots/tabs04-dev-01-routed.png docs/superpowers/plans/night-shots/tabs04-dev-02-show-after-typing.png docs/superpowers/plans/night-shots/tabs04-dev-03-bg-edit.png docs/superpowers/plans/night-shots/tabs04-dev-04-ask-waiting.png docs/superpowers/plans/night-shots/tabs04-dev-05-quick-look.png docs/superpowers/plans/night-shots/tabs04-dev-06-human-open.png
```
...
- **Moving a tab between windows (plan 05) must re-key the inbox and `AiPending`**: today "to new windows" releases the tab, and its waiting agents get `tab released`.
