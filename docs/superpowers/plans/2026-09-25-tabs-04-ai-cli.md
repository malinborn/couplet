...
| D14 | **Finder / `RunEvent::Opened` — tabs-questions Q4, reversible default:** steps 2–4 of the agent routing in the foreground — already open → there; a live window of the file's project → a new tab there (most recently focused); an empty `main` → `main`; else a new window. One function, `window::route_opened_file`; flipping to "always a new window" is deleting its project branch. | Q4 option 2, recommended by the brief: double-clicking five files of one repo gives one window, not five, and keeps windows single-project. It deliberately differs from the human CLI (D13), which the spec pins to "new window". |
...
| Finder (Q4) | §4 is about the CLI | project routing, foreground, reversible (D14) |
...
- `window.rs` — modify: restore binds the project (Task 1); `Activation`, `build_window`, `hand_over_file`, `activate_app`, `open_file_window_with`, `reveal_window` (Task 3); `partition_held`, `open_files_window`, `route_opened_file` rewrite (Task 6).
...
## Task 6: The human's open and Finder (Rust)

Spec §4: a human's `mdmini a.md b.md` opens one new window with the files as tabs. Finder (Q4): the project routing's steps 2–4 in the foreground, as a reversible default in one function.

**Files:**
- Modify: `src-tauri/src/window.rs` (`OpenedRoute` / `route_opened_file` l.672–707, tests l.783–875; new `partition_held`, `open_files_window`)
- Modify: `src-tauri/src/lib.rs` (single-instance l.105–120, `RunEvent::Opened` l.478–510, `open_pending_files` l.671–688, `load_pending_open_files` l.692–722, `handle_cli_args` l.727–751)

- [ ] **Step 1: Write the failing tests**

In the `tests` module of `src-tauri/src/window.rs`, replace the six `route_opened_file_*` tests with:

```rust
    /// `reg`, with each window bound to the project given.
    fn with_projects(mut reg: TabRegistry, projects: &[(&str, &str)]) -> TabRegistry {
        for (label, project) in projects {
            reg.bind_project(label, project.to_string());
        }
        reg
    }
    ...
    fn route_opened_file_ignores_a_mapping_to_a_dead_window() {
        let reg = reg(&[("/tmp/x.md", "editor-2")]);
        assert_eq!(route_opened_file(&reg, "/tmp/x.md", "/tmp", &[], |label| label != "editor-2"), OpenedRoute::UseMain);
    }

    #[test]
    fn route_opened_file_uses_an_empty_main() {
        let reg = with_projects(reg(&[("/q/b.md", "editor-2")]), &[("editor-2", "/q")]);
    ...
    #[test]
    fn route_opened_file_opens_a_new_window_when_main_shows_another_project() {
        let reg = with_projects(reg(&[("/q/b.md", "main")]), &[("main", "/q")]);
        assert_eq!(route_opened_file(&reg, "/tmp/x.md", "/tmp", &[], |_| true), OpenedRoute::NewWindow);
    }

    #[test]
    fn route_opened_file_gives_a_file_to_a_window_of_its_project() {
        // Q4 default: the project window wins over an empty main, and the
        // window focused last wins among several.
        let reg = with_projects(
            reg(&[("/p/a.md", "editor-2"), ("/p/b.md", "editor-3")]),
            &[("editor-2", "/p"), ("editor-3", "/p")],
        );
        assert_eq!(
            route_opened_file(&reg, "/p/docs/c.md", "/p", &["editor-3".to_string()], |_| true),
...
Run: `CARGO_TARGET_DIR=~/.cargo/tabs-impl-target cargo test --manifest-path src-tauri/Cargo.toml route_opened_file partition_held 2>&1 | tail -20`
Expected: compile errors — `route_opened_file` takes 3 arguments, `no variant ProjectWindow`, `cannot find function partition_held`.

(`cargo test` takes one filter; run `… cargo test --manifest-path src-tauri/Cargo.toml window::tests 2>&1 | tail -20` if your cargo refuses two.)

- [ ] **Step 3: Implement in `window.rs`**

Replace `OpenedRoute` and `route_opened_file` with:

```rust
/// Where a file handed to the app by the OS (`RunEvent::Opened` — Finder,
/// `open`) goes.
#[derive(Debug, PartialEq, Eq)]
pub enum OpenedRoute {
    /// A live window already shows the file: bring it forward, touch nothing.
    FocusExisting(String),
    /// A live window of the file's project takes it as a new tab.
...
/// absent.
///
/// **tabs-questions Q4 — reversible default.** Finder follows the agent
/// routing's steps 2–4 (spec §5), in the foreground: the file's own tab, else
/// a window of its project (the one focused last), else an empty `main`, else
/// a new window. To go back to "a new window unless main is empty", delete
/// the `ProjectWindow` branch below; nothing else depends on it.
///
...
pub fn route_opened_file(
    ...
    if let crate::routing::Route::Existing(label) =
        crate::routing::route(reg, path, None, file_project, focus_order, &is_live)
    {
...
`RunEvent::Opened` — replace the whole `for url in &urls { … }` body with:
...
```rust
                for url in &urls {
                    ...
                    // Walks the file system: before the registry lock.
                    ...
                    match route {
                        // The file may sit in a background tab there, or be
                        // new to a window of its project; that window opens
                        // (or activates) it through its own open path.
                        window::OpenedRoute::FocusExisting(label) | window::OpenedRoute::ProjectWindow(label) => {
...
Run: `CARGO_TARGET_DIR=~/.cargo/tabs-impl-target cargo test --manifest-path src-tauri/Cargo.toml 2>&1 | grep -E "^test result|FAILED|panicked"`
Expected: `test result: ok. 525 passed; 0 failed; 3 ignored` (523 + 2: six route tests rewritten in place, `route_opened_file_gives_a_file_to_a_window_of_its_project` and `partition_held_…` new).
...
git commit -m "$(cat <<'EOF'
feat(tabs): a human's open is one window of tabs; Finder routes by project

mdmini a.md b.md opens one new window with both as tabs (spec §4), launch
arguments become tabs of main. Finder: the file's tab, else a window of its
project, else an empty main, else a new window — tabs-questions Q4 default,
isolated in route_opened_file.
...
```markdown
- **Решено по умолчанию в плане 04 (обратимо):** вариант 2. Finder и `open` идут по маршрутизации §5 без `-t` и на переднем плане: файл уже открыт → его вкладка; есть окно проекта файла → новая вкладка там (последнее в фокусе); пустой `main` → он; иначе новое окно. Человеческий `mdmini a.md b.md` — по спеке §4: одно новое окно, файлы вкладками. Откатить Finder к «новое окно» — удалить ветку `ProjectWindow` в `window::route_opened_file` (`src-tauri/src/window.rs`), больше ничего от неё не зависит.
```
...
replaces the interim tabs section; CLAUDE.md gotchas; Q4 decided, Q6–Q8.
...
Finder (`RunEvent::Opened`) is not driven live: every way to deliver an Apple Event to a process goes through Launch Services by bundle, which resolves the installed app. Its routing is covered by `route_opened_file`'s unit tests (Task 6).
...
| Mandatory 7 — Finder per Q4, isolated, reversible | D14; Task 6 (`route_opened_file`), Task 14 (Q4 note) |
...
- **Finder is not tested live** (Task 15): only `route_opened_file`'s unit tests cover Q4's default.
