...
| Текст заметок | `.md`-файлы в папке тайника, запись через `atomic_write::save`; путь в БД | данные |
| Поиск по тексту (FTS5), позже векторы (`sqlite-vec`) | та же SQLite | производное, пересобирается |
...
| Клавиша | Действие |
|---|---|
| `⌃T` (кандидат) | отложить текущий документ — обработчик на странице, не пункт меню |
...
| Метод (MCP и CLI) | Что делает |
|---|---|
| `stash_list` | вид, заголовок, теги, время «отложено»; фильтр по тегу и тексту |
| `stash_get` | текст заметки или путь файла |
| `stash_add` | новая заметка из текста (в CLI — через stdin) или ссылка на файл; с тегами |
...
1. Папка заметок: `~/Documents/couplet/` (видна в Finder и Spotlight, переживает `brew uninstall --zap`) или скрытая в Application Support.
2. Клавиши: `⌃T` для «отложить»; нужна ли клавиша, открывающая тайник сразу.
3. Фаза 0 — отдельный срочный патч 2.0.2 до тайника: сборщик мусора сессии переносит черновики в корзину вместо удаления, и чинится причина их потери при обновлении.
