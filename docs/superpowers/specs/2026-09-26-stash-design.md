...
| Текст заметок | `.md`-файлы в `~/couplet/` (dev: `~/couplet-dev/`), запись через `atomic_write::save`; путь в БД | данные |
...
- **Папка заметок — `~/couplet/`**: видна в Finder, ищется Spotlight, переживает `brew uninstall --zap`, и корень домашней папки не защищён системой приватности macOS — нет запроса доступа, который у ad-hoc подписанного приложения повторялся бы после каждого обновления (сначала выбрали `~/Documents/couplet/`, заменили 2026-09-27 по этой причине). Для dev-сборки — `~/couplet-dev/`, чтобы dev не писал в заметки установленного приложения.
