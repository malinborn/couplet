...
| `{"ok":false,"error":"switched away from this document"}` (`show`, `edit`, `ask`) | The request had been delivered to the window, but the user replaced that window's document (Cmd+O, Recent Files, a dropped file) before it was answered — for `ask`, before an option was clicked. | Re-send once the file is open again; `mdmini show <file>` reopens it. |
| `{"ok":false,"error":"window is switching documents"}` (`show`, `edit`, `ask`) | The request arrived while the window was in the middle of replacing its document. | Retry — the switch finishes within moments; if the file is still wanted, it may need reopening first. |
