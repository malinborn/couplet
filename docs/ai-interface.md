...
Every window has a number (`#7` in its title) and a **project**. The number is the lowest one no live window holds when the window is made, so a closed window's number goes to the next new window: an old `window_binding` / `-t N` reaches whatever window holds `N` now. The project is the git toplevel of the first file it held (a worktree is its own project; outside git, that file's directory). It is bound once, lazily (`routing::bind_missing_projects`), never rebound when that file closes, and restored with the session.
