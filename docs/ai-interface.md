...
- **Walking the marks:** ⌘' jumps the caret to the next place an agent left in the document, ⌘⇧' to the previous one, wrapping around (AI → "Next AI Mark" / "Previous AI Mark", `ai-mark-nav.ts`). A mark is the start of an `edit` highlight (touching spans count as one), a comment thread's anchored fragment (its card, once the fragment is gone) — every open thread, including one the human started and the agent has not answered yet — or an `ask` question. A fold hiding the target is opened; with no marks the keys do nothing.
