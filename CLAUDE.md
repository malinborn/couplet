...
src/                    # Frontend (Svelte + TypeScript)
  ...
  lib/tabs/             # Tabs, B-lite: only the active tab is a live state
    tab-model.ts        # Pure tab list operations (insert after active, close → neighbour, ⌘1–9, ⌃Tab)
...
- **`npm run test` overcounts:** vitest picks up stale copies under `.claude/worktrees/`. Use `npx vitest run --dir src` for a true count (1450 after tabs plan 02).
