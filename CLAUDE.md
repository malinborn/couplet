...
src/                    # Frontend (Svelte + TypeScript)
  ...
  lib/tabs/             # Tabs, B-lite: only the active tab is a live state
    ...
    TabNotch.svelte / TabDrawer.svelte / TabCard.svelte # Notch (#N, count, ⌘J) + drawer (hover/⌘J, filter, sorts, ⇧-selection, drag, drag-out to the carousel, ⌘M / «В окно…»); thin DOM over the pure drawer-* modules
...
- **The carousel and the drawer.** A hover-opened drawer does not close while the carousel is up: the carousel lies outside `.drawer-wrap`, so the pointer on its way to a thumbnail has "left" the drawer; `closeCarousel` re-schedules the hover close. While a card is dragged, every key except Esc (which cancels) is swallowed — nothing may change the query, and with it the drop target, under the card. While ⌘M's carousel is up it owns every key, except during the «got» pulse after a pick; a press off the thumbnails cancels it and gives the keys back to the list.
- **⌘M is a drawer key («В окно…»), like ⌘L/⌘R/⌘U** — `DRAWER_MOVE_KEY` in `drawer-keys.ts`. It and the selection bar's «В окно…» act only inside the drawer; outside it ⌘M does nothing, and nothing in the menu claims it: `drawer-keys.test.ts` fails if `menu.rs` ever does, or gains a predefined Minimize (which carries ⌘M without an accelerator string).
