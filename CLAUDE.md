...
src-tauri/src/          # Rust (Tauri backend)
  ...
  window.rs             # Window creation with cascade + file dedup; get_window_init (what a window shows on mount); PendingFiles; FileWatchers (records the watched path); reveal_other_window; window_set_number (the notch's rename)
...
src/                    # Frontend (Svelte + TypeScript)
  ...
  lib/tabs/             # Tabs, B-lite: only the active tab is a live state
    ...
    window-number.ts    # The notch's rename of #N: parse, `window_set_number`, «Номер #N занят»
