...
src/                    # Frontend (Svelte + TypeScript)
  ...
  lib/tabs/             # Tabs, B-lite: only the active tab is a live state
    ...
    controller.ts       # The one path that changes what a window shows (open/activate/new/close/init; AI via runExclusive); reorder/closeTabs/moveToNewWindows for the drawer
...
- **`npm run test` overcounts:** vitest picks up stale copies under `.claude/worktrees/`. Use `npx vitest run --dir src` for a true count (1609 after tabs plan 03).
...
- **`runExclusive` holds the tab queue: inside it never call a queued method.** AI commands run in the controller's serial queue so a command arriving mid-switch waits instead of failing. A queued method (`openPath`, `activate`, …) awaited from inside `runExclusive` waits for that very slot and never runs. `activateNow` and synchronous non-queued helpers (`findByPath`, `markUnviewedNow`) are fine. A command that waited behind a switch may already have been answered (its tab was left or closed) — `handleAiCommand` asks `ai_is_pending` first and drops it if not.
