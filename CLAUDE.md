...
- **Never click a dev build's menus through System Events, not even "by PID".** `tell application "System Events" to … first process whose unix id is <dev pid>` resolves the process by *name*, and the dev and release binaries are both called `md-mini` — the reference silently lands on the **installed** app with the owner's documents. Measured 2026-09-25 (it returned the release pid). Drive native menus with the Accessibility API on the pid itself (`AXUIElementCreateApplication(pid)`) behind a guard that checks the pid's executable is your own `CARGO_TARGET_DIR/debug/md-mini`, and never send OS-level keystrokes: they go to whichever app is frontmost.
...
- **On-disk state must go through `paths::app_data_dir()`,** never `dirs::data_dir().join("md-mini")`. The directory is named after the product name, so a dev build gets `md-mini-dev/` and cannot overwrite an installed release app's `recovery/` (which holds the user's unsaved work) or `session-v2.json`. `paths::init` runs as the first statement in `setup`, before anything reads or writes.
