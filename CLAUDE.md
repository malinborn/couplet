...
- **`runExclusive` holds the tab queue: inside it call only `activateNow`.** AI commands run in the controller's serial queue so a command arriving mid-switch waits instead of failing. A queued method (`openPath`, `activate`, …) awaited from inside `runExclusive` waits for that very slot and never runs. A command that waited behind a switch may already have been answered (its tab was left or closed) — `handleAiCommand` asks `ai_is_pending` first and drops it if not.
