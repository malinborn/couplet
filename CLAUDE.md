...
- **Lock orders:** `ClosedStack` → `OpenFiles`; `OpenFiles` → `PendingFiles` → `FileWatchers`; `OpenFiles` → `WindowNumbers`. Never the reverse: don't lock `ClosedStack` or write the window counter to disk while holding `OpenFiles` — read what you need, drop the guard, then act (as the `Destroyed` handler does).
