...
- **⌦ / ⌫ close the ⇧-selection** (`drawerKeyAction`'s `close-selected`, then `closeSelected` — the «Закрыть выбранные» button's own path). Matched on `e.code` with any modifiers (⇧ is often still held; ⌘⌫ means nothing in the drawer). ⌫ only while the query is empty — with text it edits the query. Off during a drag, while the carousel is up (the `canCloseSelection` argument) and while the notch number is edited (its keys never reach `drawerKeyAction`). The button carries the keys as `title` + `aria-keyshortcuts`, like «В окно…».
...
- **A session restore keeps every window's number.** `restore_pending` collects the snapshots' numbers as `reserved` and first moves `main` off one of them while it is still as it started (no project, no file tab: `make_room_for_restore`), emitting `window-number` to it for its title and notch. A restored window whose own number another live window holds falls back past `reserved`, so it cannot take a later snapshot's number. Without this, #1 A and #2 B came back as #2 and #3 behind main's #1, and `-t 2` hit the old #1. `number_window` never renumbers a window that has a number (its `get_window_init` may get there first) and ignores its own and dead windows' entries.
