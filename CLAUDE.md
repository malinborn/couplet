...
src-tauri/src/          # Rust (Tauri backend)
  ...
  window.rs             # Window creation with cascade + file dedup; get_window_init (what a window shows on mount); PendingFiles
  ...
  routing.rs            # Agent routing (spec §5): binding → open tab → project window (last focused) → empty main → new window; window projects; ls listing
  ...
  ai_socket.rs          # AI interface: command socket (show/edit/ask/open/close/windows) + CLI client (ls, close, routed opens) + the agent snippets
...
- **`npm run test` overcounts:** vitest picks up stale copies under `.claude/worktrees/`. Use `npx vitest run --dir src` for a true count (1763 after tabs plan 04).
...
- **The window init contract.** Every listener that can deliver a tab (`open-file`, `reopen-tab`, `ai-command`) is registered — awaited — before the frontend calls `get_window_init`: Rust counts the window as mounted from that call on and sends events instead of a payload, and an event with no listener yet is lost. The tabs `get_window_init` returns are already registered: the frontend calls `tab_activate` for the active one (that points the watcher at it) and never `tab_open`. A tab arriving by event is not registered and goes through `tab_open`. Tab sources wait until `tabs.init` is queued (`tabSourcesReady` in `App.svelte`) — the serial queue then runs them after it; one that ran first would work on an empty tab list that init then publishes over.
