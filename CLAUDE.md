...
- **Renaming `#N` from the notch.** With the drawer open, a click on `#N` waits `DOUBLE_CLICK_MS` (400) before it does what a notch click does — the first click of a double-click would otherwise close a pinned drawer; the rest of the notch acts at once. The input is a sibling of the notch button (an `<input>` inside a `<button>` is invalid), inside the drawer root, so `insideDrawer` holds for it. The drawer's capture-phase `keydown` returns for `.notch-edit` targets, and the input stops propagation of its own keys. Starting an edit pins a hover-opened drawer. `window_set_number` decides under the registry lock (a dead window's entry does not count as holding the number) and writes the session after dropping it; routing, `ls` and the carousel read the registry, so they follow without a push. Other windows learn the change on their next read — nothing is broadcast.
