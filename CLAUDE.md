...
src-tauri/src/          # Rust (Tauri backend)
  ...
  window.rs             # Window creation with cascade + file dedup; get_window_init (what a window shows on mount); PendingFiles; FileWatchers (records the watched path); reveal_other_window
...
src/                    # Frontend (Svelte + TypeScript)
  ...
  lib/tabs/             # Tabs, B-lite: only the active tab is a live state
    ...
    TabNotch.svelte / TabDrawer.svelte / TabCard.svelte # Notch (#N, count, ⌘J) + drawer (hover/⌘J, filter, sorts, ⇧-selection, drag, drag-out to the carousel, ⌘G / «В окно…»); thin DOM over the pure drawer-* modules
...
- **`paths::app_data_dir()` before `paths::init` silently answers the RELEASE directory** (the `FALLBACK_DIR`), with no error. So any state that reads app data at construction — `RecentFiles::load()`, `WindowNumbers::load()` — is managed inside `setup` after `paths::init`, never on the builder, and such a type must never get a disk-reading `Default`: one stray `Default::default()` and a dev build loads the installed app's list.
- **Single-instance callbacks can run before `setup` has managed state.** The plugin's callback fires on its own task and may open a window while `setup` is still running. Use `app.try_state::<T>()` there and in anything it calls (e.g. `window.rs`'s `allocate_from`): `state()` panics, and a panic there ends the CLI listener for the rest of the run. A window created that early stays unnumbered until its frontend calls `get_window_init`.
...
- **Lock orders:** `ClosedStack` → `OpenFiles`; `OpenFiles` → `PendingFiles` → `FileWatchers`; `OpenFiles` → `WindowNumbers`; `OpenFiles` → `AiQueue`; `OpenFiles` → `AiPending` (`tab_move` re-labels the moved files' requests under the registry lock; nothing takes `AiPending` and then the registry — `ai_respond` and `ai_forward` drop the registry guard first). Never the reverse: don't lock `ClosedStack` or write the window counter to disk while holding `OpenFiles` — read what you need, drop the guard, then act (as the `Destroyed` handler does). `AiQueue` is pushed under the registry lock on purpose (`ai_socket::deliver`): whether a window has mounted and whether its command is queued must be one decision, or a command queued after `ai_pull_pending` is never pulled and one emitted before the mount is lost.
...
- **The drawer's ⌘L/⌘R/⌘U and ⌘G are webview keys, not menu items.** A capture-phase `keydown` on `window`, installed only while the drawer is open, owns them (and every letter, for type-to-filter) and stops propagation, so CodeMirror never sees them — ⌘U is its `undoSelection`, ⌘G its `findNext`. They work even in a hover-opened drawer that has not taken the keyboard (`actionAllowed`), or they would run in the hidden editor. `drawer-keys.test.ts` fails if `menu.rs` ever claims one: a native accelerator is resolved before the webview and the drawer would silently stop sorting. ⌘J *is* a menu item (`toggle_drawer`).
...
- **Quick-look expiry never closes the active tab of any window**, focused or not: md-mini usually sits unfocused beside the agent's terminal while the human reads it. Only background quick looks expire (clean by construction, closed the ⌘W way); an unseen one never does; the hour starts at the first view. Not persisted across a restart (tabs-questions Q8).
