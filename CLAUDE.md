...
- **A hover-opened drawer does not move focus; a pinned one does.** A pinned drawer (⌘J, notch click) always takes the keyboard. A hover-opened one takes it only if the last key or press before it went somewhere other than an editable field (tabs-questions Q5); otherwise `DrawerState.typing` stays off until a `pointerdown` or `wheel` inside the drawer, a notch click, ⌘J (which pins a hover-opened drawer instead of closing it), or a *dwell*: the pointer has travelled `DWELL_ARM_PX` (20) from where it was at the hover-open and then rests `DWELL_CAPTURE_MS` (500). Merely entering the drawer does not count — the hover-open slides the drawer under a pointer resting on the notch. Until then Esc goes to the editor too (`actionAllowed`); a hover drawer closes by the pointer leaving. Closing restores focus only if it was inside the drawer.
