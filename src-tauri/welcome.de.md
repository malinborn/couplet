...
| | |
|---|---|
| **show** | Zu einer Zeile oder Textstelle springen und sie kurz aufblitzen lassen. |
| **edit** | Das offene Dokument direkt umschreiben; die geänderte Stelle bleibt markiert, bis du **Esc** drückst. |
| **ask** | Echte Schaltflächen ins Dokument setzen — eine Option, mehrere, oder ein Textfeld — und auf deine Antwort warten. |
| **question** / **answer** | Kommentare lesen, die du an einer Stelle hinterlässt (**⇧⌘M**), und im selben Thread antworten. |

Nichts läuft hinter deinem Rücken: **⌘Z** macht eine KI-Änderung genauso rückgängig wie deine eigene Eingabe, und Kommentar-Threads liegen in einer eigenen Markdown-Datei neben dem Dokument, nicht darin.
...
**Zwanghafte Ausrichtung** — **Ansicht → Zwanghafte Ausrichtung**. Das Häkchen in einer aktivierten Box sitzt ein Haar neben der Mitte. Das ist kein Schlampigkeitsfehler: `✓` ist einfach kein symmetrisches Zeichen und wirkt deshalb nie zentriert, egal was man versucht. Wenn dich dieses Haar den Rest des Tages nicht mehr loslässt, ersetzt dieser Schalter es durch ein Kreuz aus zwei Strichen, die um denselben Punkt gedreht sind — von Natur aus symmetrisch, mit nichts mehr, das verrutschen könnte. Wir mögen das ✓, so wie es ist. Aber wir verstehen die Marotte.
