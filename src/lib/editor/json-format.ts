 ...
 * arrives here through the clipboard. `JSON.stringify(JSON.parse(s), null, 2)`
 * does the actual work and needs no dependency; everything below exists to
 * decide *whether to offer*, which is the part that can be wrong in ways a
 * user notices.
 */
 ...
 *
 * `JSON.parse` on a multi-megabyte string blocks the only thread the editor
 * has, and it would run on every single paste. The cap is generous enough for
 * any log line or database column and cheap to check.
 */
 ...
 * - **Already-expanded JSON.** Round-tripping it changes nothing, so the offer
 *   would be a toast with a button that does not do anything.
 *
...
export function analyzeJson(text: string): JsonAnalysis | null {
  ...
  // Cheap structural gate before the parser: rejects prose, markdown and
  // scalars without paying for a parse on every keystroke-sized paste.
  ...
  // Defensive: the bracket gate above already implies this, but a parse result
  // is what the formatter actually operates on.
  if (parsed === null || typeof parsed !== 'object') return null;

  const formatted = JSON.stringify(parsed, null, 2);
  if (formatted === undefined) return null;

  return { formatted, alreadyFormatted: formatted === trimmed, shape };
}
